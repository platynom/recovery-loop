...
This is the camera-side reference for the final demo. Evaluation values come from `data/evaluation/fix7-npci-calibrated.json`; external constraints and benchmarks link to their primary sources. Figures labelled runtime-simulated are illustrative decision inputs, not performance claims.
...
Removed from camera-facing material: the old ONS value, the unsupported peak-window candidate multiplier, and circular-evaluation headline values that were not retained in committed machine-readable evidence.
