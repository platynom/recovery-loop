...
export async function GET(request: Request) {
  ...
  const evaluation = evaluatePolicies(options);
  return Response.json({
    mode: 'labeled-simulation',
    policy: { coverageThreshold, monthlyBudget },
    decisions: run.decisions.slice(0, 30).map((decision, index) => ({ ...decision, event: run.events[index] })),
    metrics: {
      ...
      refused: run.decisions.filter((decision) => decision.action === 'refuse').length,
